...
.reserve(int)               & Pre-allocate memory for performance\\
...
\begin{itemize*}
    ...
    \item \texttt{\#pragma omp parallel} — OpenMP parallel region (requires OpenMP enabled by compiler flags).
    \item \texttt{\#pragma intrinsic(func)} — request compiler intrinsic (MSVC).
\end{itemize*}
...
\begin{tabular}{>{\small\ttfamily}ll}
    ...
    -gline-tables-only  & Minimal debug info for profilers.\\
    -O0, -O1,           & Optimizations levels, where \texttt{-O0} is no\\
    -O2, -O3,           & optimization\\
    -Ofast
