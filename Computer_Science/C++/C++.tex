...
\begin{center}
    ...
    \small{Class: CSCI 1300 \& COSC 1550 \& COSC 1560}\hfill\small{\textcopyright Maximilien Notz \the\year{}}
...
std::move(obj)          & During assignation move the already\\
                        & existing object instead of creating a\\
                        & copy of it (memory optimizations).\\    
typedef T A             & Create an alias that refers to the type \texttt{T}. \\  
...
.substr(a,b)        & Returns the substring starting at index \texttt{a} with length \texttt{b}.\\
.find(subStr)       & Return the start index of the substring\\
.replace(i,l,str)   & Replace $l$ characters starting at index \texttt{i} with \texttt{str}.\\
...
to\_string(var)     & Convert \texttt{var} to a string,\\
\end{tabular}
...
v[i]                        & Get or set the element at index \texttt{i}.\\    
.at(i)                      & Returns the element at index \texttt{i} and performs a\\
                            & bound check. \\
...
.reserve(size)              & Pre-allocate memory for performance, \\
                            & Use it when the maximum size is known. \\
.front()                    & Returns the first element. \\
.back()                     & Returns the last element. \\
.empty()                    & Returns true if vector is empty. \\
.erase(it)                  & Erase element at iterator \texttt{it}. \\
\end{tabular}
...
\subsubsection{fstream functions}
\begin{tabular}{C L}
getline(fin, line)      & Get the next line from \texttt{fin} and assign it to \texttt{line}. \\
.open("file.txt")    & Open txt file whith the stream. \\
.is\_open()          & Return true if the file is open. \\
...
ios::trunc     & Empty the file if it exists.\\
ios::ate       & Open and start at the end of the file.\\
ios::binary    & Open in binary mode (no text processing).\\
\end{tabular}

\footnotesize{File modes can be combined using the bitwise OR operator \texttt{|}.}
...
<<showpoint<<           & Enables or disables the unconditional inclusion of the \\
<<noshowpoint<<         & decimal point character in floating-point output. \\
...
\begin{tabular}{C L}    
    ...
    p->m                    & Access member \texttt{m} of the object pointed to by \texttt{p}.\\
\end{tabular}

\subsection{Smart Pointers}
\begin{tabular}{C L}   
    \#include<memory> & Include memory library.\\
    unique\_ptr<T> uptr       & Unique ownership smart pointer, only one\\
                              & pointer can point at the variable.\\
    make\_unique<T>(args)     & Create a unique\_ptr instance.\\
    shared\_ptr<T> sptr       & Shared ownership smart pointer, multiple \\
                              & pointers can point at the variable.\\
    make\_shared<T>(args)     & Create a shared\_ptr instance.\\
    .use\_count()             & Get the number of shared\_ptr instances managing \\
                              & the same object.\\

    weak\_ptr<T> wptr         & Non-owning smart pointer (observe only), used to break\\
                              & circular references with shared\_ptr.\\
    .lock()                   & Get a shared\_ptr from a weak\_ptr. Returns an empty\\
                              & shared\_ptr if the managed object has been destroyed.\\
    wptr.expired()            & Check if the managed object has been destroyed.\\
\end{tabular}
...
\newpage
\subsubsection{Advanced Memory Management}
\begin{tabular}{C L}
    malloc(size)              & Allocate \texttt{size} bytes of memory.\\
    calloc(n, size)           & Allocate memory for an array of \texttt{n} elements of \texttt{size}\\
                              & bytes each, initialized to zero.\\
    realloc(ptr, size)        & Reallocate memory to a new size.\\
    free(obj)                 & dynamically de-allocate the memory allocated with\\
                              & \texttt{malloc}.\\
\end{tabular}
...
\begin{tabular}{C L}
    ...
    random\_device              & \\
    mt19937                     & Linear distributed randome number generator object.\\
    uniform\_real\_distribution & Uniform randome number generator object.\\
    normal\_distribution        & Normal randome number generator object.
\end{tabular}
...
\begin{tabular}{>{\small\ttfamily}ll}
    ...
    -c filesName        & generate an object file. To add \texttt{.o} filse to\\
                        ...
                        & regular file.\\  
    -O0, -O1,           & Optimizations levels, where \texttt{-O0} does not\\
    -O2, -O3,           & optimize the code.\\
    -Os                 & Optimize for size.\\
    -fast-math          & Enable fast unsafe math optimizations.\\
    -march=native       & Optimize code for the architecture of the\\
                        & machine on which the compiler is running.\\
    -std=c++17          & Specify version to C++ 17.\\
    -Wall               & activates all default warnings\\
    -Wextra             & Enable extra warnings beyond \texttt{-Wall}.\\
    -Wpedantic          & Reject all programs that use forbidden extensions.\\
    -Werror             & Make all warnings into errors.\\
    -Wshadow            & Warn for variables that shadow other variables.\\
    -Wconversion        & Warn for implicit type conversions that may alter a\\
                        & value.\\
    -Wcast-qual         & Warn whenever a pointer is cast so as to remove a\\
                        & type qualifier from the target type.\\
